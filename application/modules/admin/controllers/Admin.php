...
class Admin extends Admin_Controller {
    ...
    public function delete_user() {
     ...
     public function delete_influencer() {
        ...
        if ($data) {
            ...
            if ($type == 0) {
                redirect(base_url('admin/influencerList'), 'refresh');
            } else {
                redirect(base_url('admin/artistList'), 'refresh');
            }
        }
    ...
    public function influencerList() {
        ...
        $this->adminHtml('Influencer List', 'influencer-list', $data);
    }
    ...
    public function edit_user() {
        ...
        if ($this->form_validation->run() == false) {
            $data['user'] = $this->common->getData('user', array('id' => $user_id), array('single'));
            $data['language'] = $this->common->getData('language', array(), array());
     ...
     public function edit_influencer() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->adminHtml('Update Influencer Details', 'add-influencer', $data);
        } else {
            unset($_POST["submit"]);
            $id = $this->input->post('id');
           unset($_POST["id"]);
    ...
    public function show_postlist() {
      ...
      public function delete_post() {
        ...
        if ($data) {
            ...
            redirect(base_url('admin/postList'), 'refresh');
        }
    }
    ...
    public function add_influencer() {
        $this->form_validation->set_rules('full_name', 'Influencer name', 'required');
        ...
        $this->form_validation->set_rules('password', 'Password', 'required');
         $data['language'] = $this->common->getData('language', array(), array());
        if ($this->form_validation->run() == false) {
            $this->adminHtml('Add Influencer', 'add-influencer', $data);
        } else {
               if(isset($_FILES['profile_image'])){
                $image = $this->common->do_upload('profile_image','./assets/userfile/profile/');
                if (isset($image['upload_data'])) {
